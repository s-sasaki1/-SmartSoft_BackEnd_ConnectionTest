    ...
    public class ConnectionSmartSoft : ControllerBase
    {
        ...
        void MessageDeserialized(IMessageParameters? MessageParam, string Message, int ErrorCode)
        {
            ...
            if (ErrorCode == 0)
            {
                ...
                if (MessageParam.Datas.RetunrData != null)
                {
                    //ReturnEventErrorCode = MessageParam.Datas.RetunrData.ErrorCode;

                    //ReturnEventErrorCodeが0ならば、SmartSoft側で正常に終了
                    //それ以外は、エラー

                    //測定の場合
                    if (AcqSendGUID == MessageParam.Datas.RetunrData.UID)
                    {
                        //測定終了メッセージ

                        foreach (Group ci in MessageParam.Datas.Groups)
                        {
                            if (string.Compare(ci.GroupName, "AutoTool", true) == 0)
                            {
                                foreach (KeyValuePair<string, string> kvp in ci.Parameter)
                                {
                                    UpdateHeaderComp(kvp.Key, kvp.Value);
                                }
        ...
        private void UpdateHeaderComp(string key, string val)
        {
            //if (listView1.InvokeRequired)
            //{
            //    listView1.Invoke((MethodInvoker)delegate { UpdateHeaderComp(key, val); });
            //    return;
            //}

            //ListViewItem cur;
            //cur = listView1.Items[key];
            //if (cur != null)
            //{
            //    cur.SubItems[3].Text = val;
            //}
            //else
            //{
